...
public class GestionBookImpJDBC implements IGestionBook {
	 private List<Book> liste=new ArrayList<Book>();

	private Connection cnx;
	public GestionBookImpJDBC() throws SQLException
	{
	...
	public void ajouterBook(Book e) throws SQLException {
		PreparedStatement ps=cnx.prepareStatement("INSERT INTO Books(BookID,Title,Publisher,Category,CoverPath,PDFPath) VALUES(?,?,?,?,?,?)");
	...
	public List<Book> rechercherParMC(String mc) throws SQLException {
		...
		PreparedStatement ps=cnx.prepareStatement("SELECT * FROM Books WHERE Title LIKE ?");
		ps.setString(1, "%"+mc+"%");
	...
	public List<Book> listeDesBooks() throws SQLException {
	    ...
	    PreparedStatement ps = cnx.prepareStatement("SELECT * FROM Books");
	    ResultSet rs = ps.executeQuery();
	    while (rs.next()) {
	        int id = rs.getInt(1);
	...
	public void supprimerBook(int bookid)throws SQLException {
		PreparedStatement ps=cnx.prepareStatement("DELETE FROM Books WHERE BookID=?");
		ps.setInt(1, bookid);
		ps.executeUpdate();
	}
